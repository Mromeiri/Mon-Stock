...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
  <div class="sidenav-header">
    ...
    <a class="navbar-brand m-0" href="{% url 'admin:index' %}">
      <img src="{% static 'img/mybacacademy.png' %}" class="navbar-brand-img h-100" alt="main_logo">
    ...
    <ul class="navbar-nav">
      <li class="nav-item">
        ...
        {% endif %}
          ...
          <h5 class="" style="padding-top: 10px;font-weight:700;font-size:15px;cursor: pointer;"  onclick="{{ app.url }}">
            <span class="nav-link-text ms-1">{% trans 'Home' %}</span>
      ...
      {% if app_list %}
          {% for app in app_list %}
              {% if app.has_perms and not app.pinned %}
                <li class="nav-item">
                  <span class="nav-link{% if app_label != app.label and app.url not in request.path|urlencode  %} collapsed{% endif %} d-flex justify-content-between align-items-center" data-bs-toggle="collapse" data-bs-target="#submenu-{{ app.app_label }}"style="cursor: pointer;">
                    <span class="d-flex justify-content-between align-items-center">
                      <div class="icon icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center" style="padding: 2px;">
                        {% if app.icon %}
                          <span class="sidebar-icon {{ app.icon }}"></span>
                        {% else %}
                          <img src="{% static 'img/'|add:app.label|add:'.png' %}" class="navbar-brand-img h-100" >
                        {% endif %}
                      </div>
                      
                      <div class="numbers" >
                        
                        <h5 class="" style="font-weight:700;font-size:15px;cursor: pointer;"  onclick="{{ app.url }}">
                    ...
                    <div class="multi-level ms-5 collapse {% if 'tables-bootstrap-' in segment or app_label == app.label or app.url in request.path|urlencode %} show {% endif %}" role="list" id="submenu-{{ app.app_label }}"                                                                                                       aria-expanded="{% if app_label == app.label or app.url in request.path|urlencode %}true{% else %}false{% endif %}">
                      <ul class="flex-column nav">
                        ...
                        <li class="nav-item{% if 'bootstrap-tables' in segment or model.url in request.path|urlencode %} active {% endif %} list-unstyled">
                          <h5 class="nav-link" style="font-weight:700;font-size:13px;cursor: pointer;" onclick="window.location='{{ model.url }}';">
